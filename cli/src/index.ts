...
const secret =
  "e617c84041f0d4f28c3ba632f175ef94babc1a5ea1c483abfd3d2030ca8950c4";
...
const SaveFileShadowDrive = async (fileUrl: any, fileName: any) => {
  const store_account = new PublicKey(
    "npv3ffY39EpGApjroKZFHX8wM1MBWcrHm41Us8F6YWS"
  ); // ask leonel for direction
  const data = new FormData();

  const file = fs.readFileSync(fileUrl);

  const SECRET_KEY: string = process.env.CLI_WALLET_PRIVATE_KEY
    ? process.env.CLI_WALLET_PRIVATE_KEY
    : "";
  const keypair = Keypair.fromSecretKey(bs58.decode(SECRET_KEY));

  const owner_store_account = new PublicKey(keypair.publicKey);
  const _filename = fileName;
  const fileHashSum = crypto.createHash("sha256");
  const fileNameHashSum = crypto.createHash("sha256");
  fileHashSum.update(Buffer.isBuffer(file) ? file : Buffer.from(file));
...
programCommand("init_config")
  ...
  .requiredOption("-i, --info <path>", "Information loacation")
  .action(async (directory, cmd) => {
    try {
      ...
      let space =
        CONFIG_SIZE + 4 + CONFIG_LINE_SIZE * infoJson.maxNumberOfLines;
      ...
      transaction.add(
        program.instruction.initConfig(
...
programCommand("update_config")
  .requiredOption("-k, --keypair <path>", "Solana wallet location")
  .requiredOption("-i, --info <path>", "Information loacation")
  .requiredOption("-c, --config <string>", "config account")
  .action(async (directory, cmd) => {
    try {
      ...
      let transaction = new Transaction();
      transaction.add(
        program.instruction.updateConfig(
          {
            symbol: infoJson.symbol,
            creator: new PublicKey(infoJson.creator),
            sellerFee: infoJson.sellerFee,
...
programCommand("add_config_lines")
  .requiredOption("-k, --keypair <path>", "Solana wallet location")
  .requiredOption("-u, --url-asset <string>", "Information loacation")
  .requiredOption("-c, --config <string>", "Config account")
  .option("-fn, --from-number <number>", "art from_number")
  .option("-tn, --to-number <number>", "art to_number")
  .action(async (directory, cmd) => {
    try {
      const { env, keypair, urlAsset, config, fromNumber, toNumber } =
        cmd.opts();
      const conn = new Connection(clusterApiUrl(env));
      const owner = loadWalletKey(keypair);
      const configPublicKey = new PublicKey(config);
      ...
      for (let i = fromNumber; i < toNumber; i++) {
        ...
        let imageRes: any = await SaveFileShadowDrive(
          imageUrl.toString(),
          `${i}.png`
        );
        ...
        if (imageRes.success && imageRes.shadowUrl) {
          ...
          let jsonRes: any = await SaveJsonShadowDrive(
          ...
          if (jsonRes.success && jsonRes.shadowUrl) {
            let transaction = new Transaction();
            transaction.add(
              program.instruction.addConfigLines(
                [
                  {
                    name: metadata.name,
                    uri: jsonRes.shadowUrl,
                  },
                ],
                {
                  accounts: {
      ...
      for (let i = fromNumber; i < toNumber; i++) {
        ...
        if (imageRes.success && imageRes.shadowUrl) {
          ...
          if (jsonRes.success && jsonRes.shadowUrl) {
            let transaction = new Transaction();
            transaction.add(
              program.instruction.updateConfigLine(
                new anchor.BN(i),
                {
                  name: metadata.name,
                  uri: jsonRes.shadowUrl,
...
programCommand("init_pool")
  ...
  .action(async (directory, cmd) => {
    try {
      ...
      const [pool, bump] = await PublicKey.findProgramAddress(
        [rand.toBuffer()],
        programId
      );
      let transaction = new Transaction();
...
programCommand("update_pool")
  .requiredOption("-k, --keypair <path>", "Solana wallet location")
  .requiredOption("-i, --info <path>", "Information loacation")
  .requiredOption("-p, --pool <string>", "pool account")
  .action(async (directory, cmd) => {
    try {
      ...
      let transaction = new Transaction();
      transaction.add(
        program.instruction.updatePool(
          new PublicKey(infoJson.updateAuthority),
          new PublicKey(infoJson.scobyWallet),
          new anchor.BN(infoJson.mintingPrice * LAMPORTS_PER_SOL),
          infoJson.royaltyForMinting,
...
programCommand("get_nfts_for_owner")
  .requiredOption("-o, --owner <string>", "owner pubkey")
  .requiredOption("-p, --pool <string>", "pool account")
  .action(async (directory, cmd) => {
      ...
      try {
        ...
        if (tokenAmount.amount == "1" && tokenAmount.decimals == "0") {
          ...
          if (metadata.data.data.symbol == "SPORE") {
            let [metadataExtended, bump] = await PublicKey.findProgramAddress(
              [nftMint.toBuffer(), poolAccount.toBuffer()],
              programId
            );
    ...
    allTokens.map((item) => {
      console.log(
        ...
        item.mint.toBase58(),
        "  =>  parent : ",
        item.extendedData.parent.toBase58(),
        "  id:  ",
        item.extendedData.number,
        "  followers cound : ",
        item.extendedData.childrenCount
      );
    });
    console.log("");
  });
